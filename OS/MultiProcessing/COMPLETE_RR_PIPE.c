...
void time_tick(int signo)
{
	...
	if(flag==0){
	//enqueue(time_q);
	write(PtoC[1],&time_q,sizeof(time_q));
	flag=1;
	}
	 if(r_pcb->head->response_flag==0){//처음실행시키는애 만남
...
void get_cputime(int signo)
{
	...
	if(flag==0){
	read(PtoC[0],&time_q,sizeof(time_q));
	flag=1;
	}
        cpu_burst--;
        time_q--;
		...
		write(CtoP[1],&io_burst,sizeof(io_burst));
...
void pop_ready()
{
	...
	ready_count--;
	read(CtoP[0],&io_burst,sizeof(io_burst));
	temp->io_burst=io_burst;
	r_pcb->head=NULL;
	r_pcb->tail=NULL;
	push_wait(temp);
	return;
...
void pop_wait(){
	...
	temp->response_flag=0;//팝할떄 0으로바꿔줌
	push_ready(temp);
	wait_count--;
...
}//확인, 처음에 burst io,cpu정해줌
void reduce_wait()//웨이트큐ㅜ 카운트줄임
{
        pcb* temp;
       // temp = (pcb*)malloc(sizeof(pcb));
        temp= w_pcb->head;
