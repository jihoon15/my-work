...
typedef struct pcb{
        ...
        int for_turnaround;
        int turnaround;
	//아마 pcb가 포인터 형식으로 dentry 갖고있는다?
	//그거에 대한 valid
	struct dentry* opened_file[100];
	int valid[100];
}pcb;
...
void push_pcb(pcb_L *L, int pid, int cpu_time, int wait_time, int form) //프로세스 생성 시 run,ready q에 push
{
        pcb *newpcb;
        pcb *temp;
