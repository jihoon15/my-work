...
void WriteMem(int Addr, int val)
{
 int tag=	(Addr&0xfffff800)>>11;//21
 int index=	(Addr&0x000007c0)>>6;//5
...
FIND(find,tag,index);//cache_index 설정
	...
	else//원하는값 없음
	{
	  ...
	  int drain_addr =(CACHE[cache_index].LINE[index].tag<<11)|(index<<6);
	  //저장할곳= cache_index
...
int ReadMem(int Addr)
{
 int tag=	(Addr&0xfffff800)>>11;//
 int index=	(Addr&0x000007c0)>>6;
