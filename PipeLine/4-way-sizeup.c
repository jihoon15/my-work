...
void WriteMem(int Addr, int val)
{
 int tag=	(Addr&0xffff0000)>>16;//16
 int index=	(Addr&0x0000ffc0)>>6;//index 10
...
FIND(find,tag,index);//cache_index 설정
	...
	else//원하는값 없음
	{
	  ...
	  int drain_addr =(CACHE[cache_index].LINE[index].tag<<16)|(index<<6);
	  //저장할곳= cache_index
