...
void ID_RF(LATCH1 latch1)
{
	...
	if((ID_EX[0].rs == ID_EX[1].Write_reg)&&(use_rs(ID_EX[0]))&&(ID_EX[1].second.RegWrite))//거리1
	{
	ALU1(ID_EX[1]);
	...
	else if((ID_EX[0].rs == EX_MEM[1].Write_reg)&&(use_rs(ID_EX[0]))&&(EX_MEM[1].third.RegWrite))//거리2
	{
	...
	if((ID_EX[0].rt == ID_EX[1].Write_reg)&&(use_rt(ID_EX[0]))&&(ID_EX[1].second.RegWrite))//거리1
	{
	ALU1(ID_EX[1]);
	...
	else if((ID_EX[0].rt == EX_MEM[1].Write_reg)&&(use_rt(ID_EX[0]))&&(EX_MEM[1].third.RegWrite))//거리2
	{
	Readdata=MEMORY_INST[EX_MEM[1].ALU1result/4];
	...
	case 4://BRANCH ** 이미 C_SIGNAL>Branch==1이다
			switch(latch2.opcode){//_______________________________________예외____________________________-
				case 0x4://beq
						...
						{
						   if(check_fact==ONE_BIT[check_index])//성공!
						   {
						        if(SECOND_BIT[check_index]==ONE_BIT[check_index])//strong
							{
						   	HIT++;
							}
							else//weak
							{
							HIT++;
							SECOND_BIT[check_index]=!SECOND_BIT[check_index];
							}
						   }
						   else//실패
						   {
							...
							{
							MISS++;
							ONE_BIT[check_index]=!ONE_BIT[check_index];
							SECOND_BIT[check_index]=!SECOND_BIT[check_index];
						  	T_ADDRESS[check_index]=t_target;
						   	NT_ADDRESS[check_index]=nt_target;
							}
						   //스위칭
						   IF_ID[0]=IF_ID[2];
						   ID_EX[0]=ID_EX[2];
...
void STORE_SLOT(LATCH2 latch2)//처음저장할떈 strong taken
{
 int i=0;
 ...
 EMPTY[i]=1;
 ONE_BIT[i]=check_fact;
 SECOND_BIT[i]=ONE_BIT[i];
 IF_ID[0]=IF_ID[2];
